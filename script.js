...
// ---- CACHED MESSAGE ----
const CACHED_MESSAGE = JSON.stringify({
  header: {
    session:
  ...
  body: {
    listOf: [
      {
        campaign: { source: null, medium: null, campaign: null, term: null, content: null },
...
let statMax = 0;
const BUCKETS = new Array(5000).fill(0); // histogram 0–4999ms

function recordLatency(ms) {
  const t = Math.min(ms, BUCKETS.length - 1);
  BUCKETS[t]++;
  statCount++;
  statSum += ms;
...
function getPercentile(p) {
  const target = Math.floor(statCount * p);
  let cum = 0;
...
(async () => {
  ...
  console.log("RPS:", (sent / totalDuration).toFixed(4));
  console.log("Latency min (ms):", statMin === Infinity ? 0 : statMin);
  console.log("Latency max (ms):", statMax);
  console.log("Latency avg (ms):", (statSum / statCount || 0).toFixed(3));
  console.log("Latency p90 (ms):", getPercentile(0.9));
  console.log("Latency p95 (ms):", getPercentile(0.95));
  console.log("========================================");
